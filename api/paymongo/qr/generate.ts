// PayMongo v1/qrph/generate API Serverless Function for Vercel (/api/paymongo/qr/generate)
...
export default async function qrHandler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // Check if Static QR Ph was requested
    const isStatic =
      mode === "static" ||
      is_static === true ||
      type === "static" ||
    ...
    // Build official PayMongo instore QR Ph payload
    ...
    // If dynamic, attach exact transaction amount in centavos
    ...
    if (!response.ok || !responseData || !responseData.data) {
      ...
      console.error("PayMongo API error in Vercel qr function:", responseData);
    ...
    console.error("PayMongo QR server error in Vercel qr function:", err);
    return res.status(500).json({
...
export { qrHandler as handler, getPayMongoAuthHeader };
